public class FlightNavPath {
  ...
  let navPathQuestFxResource: FxResource;
  let FXPoints: Int32;
  ...
  public static func Create(controller: ref<FlightController>) -> ref<FlightNavPath> {
    ...
    self.FXPoints = 100;
    self.navPathQuestFxResource = Cast<FxResource>(r"base\\fx\\camera\\focus_objects\\fx_in_world_nav.effect");
    return self;
  }

  public func Update() {
    if IsDefined(this.mmcc) {
      this.UpdateNavPathPoints(this.navPathQuestFX, this.mmcc.questPoints);
    }
  }

  private func UpdateNavPathPoints(out fxs:array<ref<FxInstance>>, points: array<Vector4>) -> Void {
      ...
      if (tweenPointDistance > this.spacing) {
        ...
        while (x <= tweenPointDistance) {
          ...
          if Vector4.Distance(correctedMidPoint, this.controller.player.GetWorldPosition()) > this.closestPoint {
            this.UpdateNavPathFX(fxs, pointsDrawn, correctedMidPoint, Quaternion.BuildFromDirectionVector(correctedMidPoint - lastFxPoint));
            pointsDrawn += 1;
            if (pointsDrawn >= this.FXPoints)
            {
              break;
    ...
    if pointsDrawn < this.FXPoints {
      while pointsDrawn < this.FXPoints {   
        this.UpdateNavPathFX(fxs, pointsDrawn, new Vector4(0.0, 0.0, -1000.0, 0.0), new Quaternion(0.0, 0.0, 0.0, 1.0));
        pointsDrawn += 1;
      }
    }
  }

  private func UpdateNavPathFX(out fxs: array<ref<FxInstance>>, i: Int32, p: Vector4, q: Quaternion) {
    ...
    if ArraySize(fxs) < (i + 1) {
      ArrayPush(fxs, GameInstance.GetFxSystem(this.controller.gameInstance).SpawnEffect(this.navPathQuestFxResource, wT0));
    } else {
      if !fxs[i].IsValid() {
        fxs[i] = GameInstance.GetFxSystem(this.controller.gameInstance).SpawnEffect(this.navPathQuestFxResource, wT0);
      } else {
